...
def figure_Me( time_me):
    ...
    plt.show()
def figure_hiertrain_dec_FEEL(time_for_figure_hiertrain,time_for_figure_eec, time_for_figure_FEEL, bandwidth):
    ...
    ax.plot(bandwidth, df[1], label='DEC', color='red', linestyle='--')
...
def figure_md_min_training(min_total_time_md, all_cloud, all_edge, hiertrain ):
    ...
    ax.set_xlabel("md layers")
    ax.set_ylabel("Training time")
    ax.plot(df.index, df[0], label='DEC', color='blue', linestyle='--')
...
def figure_me_min_training(min_total_time_me, all_cloud, all_edge):
    ...
    ax.legend()
    plt.show()
def figure_dec_all_cloud_all_edge_bandwidth(time, time_all_cloud, time_all_edge, bandwidth):
    ...
    ax.set_xlabel("bandwidth(Mbps)")
    ax.set_ylabel("Training time")
    ax.plot(bandwidth, df[0],  label='DEC', color='red', linestyle='--')
    ax.plot(bandwidth, df[1],  label='all_cloud', color='green', linestyle='--')
    ax.plot(bandwidth, df[2],  label='all_edge', color='blue', linestyle='--')
    ax.legend()
    plt.show()
def figure_dec_hiertrain_bandwidth(time, time_hiertrain, time_all_cloud, time_all_edge, bandwidth):
    df = pd.DataFrame(time)
    df.insert(1, 1, time_all_cloud)
    df.insert(2, 2, time_all_edge)
    ...
    ax.legend()
    plt.show()
def figure_dec_edge_cpus( min_total_time_bandwidth_list , bandwidth):
    ...
    ax.set_xlabel("bandwidth(Mbps)")
    ax.set_ylabel("Training time")
    ax.plot(bandwidth, df[0], label='2 CPUs', color='red', linestyle='--')
